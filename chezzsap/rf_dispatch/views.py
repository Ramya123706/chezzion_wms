...
def create_sorting(request):
    ...
    if request.method == "POST":
        ...
        # Step 2: Process SortingForm
        form = SortingForm(request.POST)
        if form.is_valid():
            sorting = form.save(commit=False)
            sorting.outbound = outbound_delivery  # Link Sorting to OutboundDelivery
            sorting.so_no = so_obj  # ✅ Fix: Assign SalesOrder instance explicitly
